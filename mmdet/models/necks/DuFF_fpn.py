...
class SpatialPreservation(BaseModule):
    """
    ...
    def __init__(self, in_planes,
                 ...
                 use_proj=True,
                 proj_norm=False):
        super(SpatialPreservation, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.statistic_type == 'V1':
            ...
            statistic = self.conv1(max_out)
        elif self.statistic_type == 'Max':
            max_out, _ = torch.max(x, dim=1, keepdim=True)
            statistic = max_out
            if self.use_proj:
                statistic = self.conv1(statistic)
        elif self.statistic_type == 'Mean':
            mean_out = torch.mean(x, dim=1, keepdim=True)
            statistic = mean_out
            if self.use_proj:
                statistic = self.conv1(statistic)
        else:
            raise Exception('Wrong statistic_type: ', self.statistic_type)
        if self.use_sigmoid:
            statistic = self.sigmoid(statistic)
        outs['statistic'] = statistic
        if self.proj_norm and self.use_proj:
            outs['loss'] = torch.abs(self.conv1.weight[0, 0, 0, 0] - x.new_ones(1))
...
class SemanticConsistency(BaseModule):
    ...
    def __init__(self,
        ...
        elif self.statistic_type == 'Linear':
            self.pool = nn.AdaptiveAvgPool2d(1)
            self.sigmoid = nn.Sigmoid()
            if self.use_proj:
    ...
    def forward(self, x):
        ...
        outs = dict()
        if self.statistic_type == 'V1':
            statistic = self.fc2(self.relu1(self.fc1(self.pool(x))))
        elif self.statistic_type == 'Linear':
            # ----- Before max
            statistic = self.pool(x)
            if self.use_proj:
                statistic = self.conv1(statistic)
                ...
                statistic = x
            # ----- After max
            # statistic = self.max_pool(statistic)

        elif self.statistic_type == 'Max':
            statistic = self.pool(x)
        else:
            raise Exception('Wrong statistic: ', self.statistic_type)
...
class AFF(BaseModule):
    ...
    def build_feat(self, feat_cfg, planes):
        ...
        if f_type == 'Spatial':
            feat_module = SpatialPreservation
            f_out_channel = 1
        elif f_type == 'Channel':
            feat_module = SemanticConsistency
    ...
    def forward(self, x, x_ex, return_loss=False):
        ...
        for src, rebuild_dict in zip(self.rebuild_src,
            ...
            org_rfs.append(feat_out['statistic'])

        x = self.trans_conv1(x)
        x_ex = self.trans_conv2(x_ex)
        cf1_out = self.cf1(x)
        cf2_out = self.cf2(x_ex)
        cf1 = cf1_out['statistic']
        cf2 = cf2_out['statistic']
...
class AFFM(BaseModule):
    ...
    def forward(self, x_input, x_ex_input, return_loss=False):
        if self.trans:
            x = self.trans_conv1(x_input)
            x_ex = self.trans_conv2(x_ex_input)
